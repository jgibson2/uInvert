...
object uInvert extends App {
  ...
  for (rec <- reader.toStream.par) {
    if(!rec.sequence.contains("N")) {
      rec.trim(20, 20) match {
        case Some(x) => {
          //println(s"ID: ${x.id}")
          //println(s"Trimmed sequence length: ${x.length()}")
          s_tree.find_inversion(x.sequence, args(2).toInt, s_arr) match {
            case Some(f) => {
              for (q <- f) for (v <- q.fwd_l to q.fwd_r) {
                //println(s"Found inversion at ${fasta_idx.getChromosomalPosition(s_arr(v)).get._1} ${fasta_idx.getChromosomalPosition(s_arr(v)).get._2} in text with ${q.differences} differences with score ${q.score}")
                val (chrom, start) = fasta_idx.getChromosomalPosition(s_arr(v)).get
                println(s"${chrom}\t${start}\t${start+x.length()}\t${x.id}\t${q.score}\t${q.differences}")
              }
            }
            case None => //println("Could not find inversion")
          }
          s_tree.find_inversion(SequenceUtils.reverseComplement(x.sequence), args(2).toInt, s_arr) match {
            case Some(f) => {
              for (q <- f) for (v <- q.fwd_l to q.fwd_r) {
                //println(s"Found inversion at ${fasta_idx.getChromosomalPosition(s_arr(v)).get._1} ${fasta_idx.getChromosomalPosition(s_arr(v)).get._2} in text with ${q.differences} differences with score ${q.score}")
                val (chrom, start) = fasta_idx.getChromosomalPosition(s_arr(v)).get
